...
class TestAsyncPyDruid(AsyncHTTPTestCase):
    ...
    def test_druid_returns_error(self):
        # given
        client = AsyncPyDruid(
            "http://localhost:%s" % (self.get_http_port(),), "druid/v2/fail_request"
        )
    ...
    def test_druid_returns_results(self):
        # given
        client = AsyncPyDruid(
            "http://localhost:%s" % (self.get_http_port(),), "druid/v2/return_results"
        )
    ...
    def test_client_allows_passing_default_parameters(self):
        # given
        client = AsyncPyDruid(
            "http://localhost:%s" % (self.get_http_port(),),
